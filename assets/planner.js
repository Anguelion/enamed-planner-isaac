...
ui.flashcardCustomFilters = ui.flashcardCustomFilters && typeof ui.flashcardCustomFilters === 'object' ? ui.flashcardCustomFilters : {mode:'overdue',area:'Todas',subject:'Todos',tag:'',recentDays:30,limit:50};
ui.flashcardCustomSessionIds = Array.isArray(ui.flashcardCustomSessionIds) ? ui.flashcardCustomSessionIds : [];
...
function ensureQuestionStatsIndex() {
  ...
  // A Missão precisa do progresso já salvo, não do texto integral de dezenas de
  // milhares de questões. Assim ela abre imediatamente mesmo antes de o banco
  // ser carregado; registros antigos sem scheduleId são reconciliados quando a
  // pessoa realmente entra em Questões/Simulados/Análise.
  if(buildStats && !questionBank.length) {
    Object.values(state.questionProgress || {}).forEach(result => {
      if(!result?.answeredAt || !scheduleById.has(result.scheduleId)) return;
      const stats = renderCache.questionStats.get(result.scheduleId) || { done:0, correct:0, rate:0 };
      stats.done += 1;
      if(result.correct) stats.correct += 1;
      renderCache.questionStats.set(result.scheduleId, stats);
    });
  ...
  // Índice pré-calculado evita filtrar todo o cronograma e normalizar novamente
  // o nome de cada aula para cada questão (50 mil x centenas de aulas).
  const scheduleByBlock = new Map();
  (state.schedule || []).forEach(item => {
    const block = String(item.block);
    if(!scheduleByBlock.has(block)) scheduleByBlock.set(block, []);
    scheduleByBlock.get(block).push({item, topic:canonicalTopic(item.topic)});
  });
  questionBank.forEach(question => {
    ...
    const entries = scheduleByBlock.get(block) || [];
    const lesson = direct && block === String(direct.block)
      ? direct
      : entries.find(entry => candidates.includes(entry.topic))?.item
        || entries.find(entry => candidates.some(candidate => candidate.length >= 12 && (entry.topic.includes(candidate) || candidate.includes(entry.topic))))?.item;
    if(!lesson) return;
...
function renderDashboardSmartRecommendation(recommendation, hasPinned=false) {
  if(!recommendation) return `<div class="dashboard-smart-recommendation is-clear"><span class="dashboard-smart-recommendation-icon">${iconSvg('success')}</span><div><span class="eyebrow">Recomendação automática</span><h3>Tudo em dia por enquanto</h3><p>Seus Baralhos Inteligentes não têm cartões disponíveis neste momento.</p></div><button class="icon-btn" data-dashboard-smart-manage>Ver baralhos</button></div>`;
  const {deck,analytics,urgency,overdue,reasons}=recommendation;
  const urgencyLabel={high:'Prioridade alta',medium:'Vale revisar',normal:'Próximo passo'}[urgency];
  return `<div class="dashboard-smart-recommendation urgency-${urgency}"><span class="dashboard-smart-recommendation-icon">${iconSvg('target')}</span><div class="dashboard-smart-recommendation-copy"><div><span class="eyebrow">Recomendado agora${hasPinned?' · entre os fixados':''}</span><span class="dashboard-smart-urgency">${urgencyLabel}</span></div><h3>${escapeHtml(deck.name)}</h3><p>Prioridade por ${escapeHtml(reasons.slice(0,3).join(', '))}.</p><div class="dashboard-smart-recommendation-stats"><span><b>${analytics.candidates}</b> na fila</span><span><b>${overdue}</b> atrasados</span><span><b>${analytics.retention===null?'—':analytics.retention+'%'}</b> retenção</span></div></div><button class="icon-btn primary" data-dashboard-smart-start="${escapeAttr(deck.id)}">Estudar recomendação</button></div>`;
}
...
function enhanceScheduleStudyIcons() {
  ...
  document.querySelectorAll('.schedule-table tbody tr').forEach(row => {
    ...
    const scheduleId=topicLine?.querySelector('[data-select-schedule-lesson]')?.dataset.selectScheduleLesson
      || topicLine?.querySelector('[data-open-schedule-questions]')?.dataset.openScheduleQuestions
...
function bindScheduleInputs() {
  ...
  document.addEventListener('click',event=>{
    ...
    const selectedLesson=event.target.closest?.('[data-select-schedule-lesson]');
    if(selectedLesson) {
      event.preventDefault();
      ui.scheduleSelectedId=selectedLesson.dataset.selectScheduleLesson;
      render();
      return;
    }
...
// Onde mora cada card, para o editor saber qual handler de edição usar.
function flashcardEditorSourceAttr(card) {
  if(card.videoId) return `data-video-card="${escapeAttr(card.videoId)}"`;
  if(card.questionId) return `data-question-card="${escapeAttr(card.questionId)}"`;
  return `data-library-card="${escapeAttr(card.id)}"`;
}
// Os cards eram guardados por vídeo, então a aula completa e o COFEXPRESS da
// mesma aula tinham listas separadas. Agora a lista é a da AULA: qualquer card
// vinculado ao scheduleId aparece nos dois, e o que se cria em um aparece no
// outro. Sem aula vinculada, cai no comportamento antigo (por vídeo).
function lessonEditorFlashcards(source, schedule) {
  // videoId identifica onde o card é gravado. Os registros crus de
  // state.videoFlashcards não o trazem (quem adiciona é manualFlashcards), e sem
  // ele o editor tratava o rascunho como card de biblioteca e perdia o que era
  // digitado.
  const drafts = (Array.isArray(state.videoFlashcards?.[source.id]) ? state.videoFlashcards[source.id] : [])
    .filter(card => !card.deletedAt)
    .map(card => ({...card, videoId: source.id}));
  if(!schedule) return flashcardsNewestFirst(drafts);
  // flashcardsForSchedule descarta cards sem conteúdo; sem isto, o card recém
  // criado pelo botão "+ Flashcard" (que nasce em branco) sumiria da tela.
  const linked = flashcardsForSchedule(schedule.id);
  const seen = new Set(linked.map(card => card.id));
  return flashcardsNewestFirst([...linked, ...drafts.filter(card => !seen.has(card.id))]);
}
function renderVideoFlashcardEditor(source, lesson, schedule) {
  if(!source) return '';
  const cards = lessonEditorFlashcards(source, schedule);
  ...
  return `${renderLessonFlashcardPanel(schedule)}<div class="flashcard-editor video-flashcards"><div class="section-title"><div><h3 class="video-flashcards-title" title="${escapeAttr(editorHelp)}">Flashcards</h3></div><button class="icon-btn primary" id="addVideoFlashcard">+ Flashcard</button></div>${cards.length ? `<div class="flashcard-editor-list">${cards.map(card=>{ const sourceAttr = flashcardEditorSourceAttr(card); const removeAttr = card.videoId ? `data-remove-video-card="${escapeAttr(card.videoId)}"` : `data-remove-lesson-card="${escapeAttr(card.id)}"`; return `<div class="flashcard-editor-item">${renderClozeAwareFlashcardFields(sourceAttr,card,'Frente: pergunta ou conceito','Verso: resposta curta')}<button class="tiny-btn" ${removeAttr} data-card-id="${escapeAttr(card.id)}" title="Remover flashcard">×</button></div>`; }).join('')}</div>` : `<div class="empty">${schedule?'Nenhum flashcard nesta aula ainda.':'Nenhum flashcard criado para este vídeo.'}</div>`}<div class="topic-source">${cards.length} ${cards.length===1?'flashcard':'flashcards'} · ${escapeHtml(schedule?.topic || lesson?.title || topic)}${schedule?' · vale para a aula completa e o COFEXPRESS':` · ${escapeHtml(topic)}`}</div></div>`;
}
...
function renderAulas() {
  ...
  // A lista da aula mistura cards de vídeo, de questão e da biblioteca: cada um
  // é gravado no seu próprio lugar.
  document.querySelectorAll('#aulas [data-library-card][data-card-id][data-card-field], #aulas [data-question-card][data-card-id][data-card-field]').forEach(input => {
    const save = event => event.currentTarget.dataset.libraryCard ? updateLibraryFlashcard(event.currentTarget) : updateQuestionFlashcard(event.currentTarget);
    input.oninput = save;
    input.onchange = save;
  });
  document.querySelectorAll('[data-remove-lesson-card]').forEach(button => button.onclick = event => {
    event.preventDefault();
    event.stopPropagation();
    if(!confirm('Remover este flashcard da aula?')) return;
    deleteFlashcardsByIds([event.currentTarget.dataset.removeLessonCard]);
    persist();
    renderAulas();
  });
...
function flashcardDueForecast(days=7) {
  const dueSeries=flashcardDueDailySeries(flashcardAllRecords(),Math.max(0,n(days)-1));
...
function flashcardIsWaiting(card) {
  if(!card?.waiting) return false;
  return !n(flashcardProgress(card).reviews);
}
function releaseWaitingFlashcards() {
  const budget = Math.max(0, n(state.flashcardSettings.newLimit) - flashcardNewToday() - flashcardReleasedToday());
  if(budget <= 0) return 0;
  const queue = flashcardAllRecords()
    .filter(flashcardIsWaiting)
    .sort((a,b) => flashcardCreatedTime(a) - flashcardCreatedTime(b))
    .slice(0, budget);
  const released = releaseFlashcards(queue.map(card => card.id));
  if(released) saveStateOnly();
  return released;
}
function flashcardReleasedToday() {
  const today = studyDateKey();
  return flashcardAllRecords().filter(card => card.releasedAt && studyDateKey(card.releasedAt) === today && !n(flashcardProgress(card).reviews)).length;
}
function releaseFlashcards(ids) {
  let released = 0;
  const stamp = new Date().toISOString();
  ids.forEach(id => {
    const card = mutableFlashcardRecord(id);
    if(!card?.waiting) return;
    card.waiting = false;
    card.releasedAt = stamp;
    card.rowVersion = n(card.rowVersion)+1;
    released++;
  });
  if(released) { renderCache.manualCards = null; renderCache.flashcardStats.clear(); }
  return released;
}
function flashcardStudyQueue(all=manualFlashcards()) {
  if(ui.flashcardCustomActive && ui.flashcardCustomSessionIds.length) {
    const completed=new Set(ui.flashcardCustomCompleted);
    const byId=new Map(all.map(card=>[card.id,card]));
    return ui.flashcardCustomSessionIds.map(id=>byId.get(id)).filter(card=>card && !completed.has(card.id) && !flashcardIsSuspended(card) && !flashcardIsWaiting(card));
  }
  ...
  const news = base.filter(card => !flashcardProgress(card).reviews && !flashcardIsWaiting(card));
...
// Lista de aulas no lugar dos decks área|assunto: é a "tela de decks" do Anki,
// com as três contagens que importam (novos liberados, aprendendo, a revisar).
function flashcardLessonDecks(all) {
  const today = localISODate(new Date());
  const decks = new Map();
  all.forEach(card => {
...
function flashcardDueDailySeries(cards=flashcardAllRecords(), days=84) {
  const today=studyDateKey();
  const endDate=new Date(`${today}T12:00:00`);
  endDate.setDate(endDate.getDate()+Math.max(0,n(days)));
  const end=localISODate(endDate);
  const byDate=new Map();
  flashcardActiveRecords(cards).forEach(card=>{
    const progress=flashcardProgress(card);
    if(!progress.reviews) return;
    let date=progress.nextReview || studyDateKey(progress.dueAt);
    if(!date || date>end) return;
    if(date<today) date=today;
    byDate.set(date,n(byDate.get(date))+1);
  });
  return byDate;
}
...
function flashcardReviewCalendar(series=flashcardReviewDailySeries(), cards=flashcardAllRecords(), futureDays=84) {
  const today = new Date(`${studyDateKey()}T12:00:00`);
  const start = new Date(today);
  start.setDate(start.getDate()-364);
  start.setDate(start.getDate()-start.getDay());
  const end = new Date(today);
  end.setDate(end.getDate()+Math.max(0,n(futureDays)));
  end.setDate(end.getDate() + (6-end.getDay()));
  const dueSeries=flashcardDueDailySeries(cards,futureDays+7);
  const cells=[];
  ...
  for(let index=0;index<totalDays;index++) {
    ...
    const due=n(dueSeries.get(iso));
    const totalRated = row.again + row.remembered;
    const retention = totalRated ? Math.round(row.remembered/totalRated*100) : null;
    const kind=date>today?'future':date<today?'past':'today';
    cells.push({date:iso,kind,completed:n(row.count),due,again:n(row.again),remembered:n(row.remembered),minutes:n(row.minutes),retention});
  }
  return cells;
}
function renderFlashcardHeatmap(series, cards=flashcardAllRecords()) {
  const cells=flashcardReviewCalendar(series,cards);
  const html=cells.map(cell=>{
    const amount=cell.kind==='future'?cell.due:cell.completed;
    const title=cell.kind==='future'
      ? `${fmtDate(cell.date)} · ${cell.due} ${cell.due===1?'card previsto':'cards previstos'} para revisar`
      : `${fmtDate(cell.date)} · ${cell.completed} ${cell.completed===1?'card revisado':'cards revisados'}${cell.retention===null?'':` · ${cell.retention}% lembrados`}${cell.minutes?` · ${Math.round(cell.minutes)} min`:''}${cell.kind==='today'?` · ${cell.due} ainda para revisar`:''}`;
    return `<button type="button" class="fc-heat-cell ${cell.kind} level-${flashcardHeatLevel(amount)}" title="${escapeAttr(title)}" aria-label="${escapeAttr(title)}" data-fc-heat-date="${cell.date}"></button>`;
  }).join('');
  return `<div class="fc-heatmap-shell"><div class="fc-heat-weekdays" aria-hidden="true"><span>Dom</span><span></span><span>Ter</span><span></span><span>Qui</span><span></span><span>Sáb</span></div><div class="fc-heatmap-grid">${html}</div></div><div class="fc-heat-legend"><span>Feitos</span>${[1,2,3,4].map(level=>`<i class="fc-heat-cell past level-${level}"></i>`).join('')}<span class="fc-heat-future-key"><i class="fc-heat-cell future level-3"></i>Próximas revisões</span></div>`;
}
...
function flashcardCardHistory(cardId) {
  return (state.flashcardSystem?.reviewLogs || [])
    .filter(log=>log?.cardId===cardId && n(log.rating || log.quality)>=1)
    .map(log=>({
      ...log,
      rating:Math.max(1,Math.min(4,n(log.rating || log.quality))),
      scheduledDays:Math.max(0,n(log.scheduledDays ?? log.after?.scheduledDays ?? log.after?.interval)),
      scheduledMinutes:Math.max(0,n(log.scheduledMinutes)),
      difficultyAfter:Math.max(0,n(log.difficultyAfter || log.after?.difficulty)),
      stabilityAfter:Math.max(0,n(log.stabilityAfter || log.after?.stability))
    }))
    .sort((a,b)=>(Date.parse(b.reviewedAt||'')||0)-(Date.parse(a.reviewedAt||'')||0));
}
function flashcardCardHistoryAnalysis(card) {
  const history=flashcardCardHistory(card.id);
  const progress=flashcardProgress(card);
  const remembered=history.filter(log=>log.rating>=2).length;
  const again=history.filter(log=>log.rating===1).length;
  const retention=history.length ? Math.round(remembered/history.length*100) : null;
  const recentSize=history.length>=4 ? Math.min(10,Math.ceil(history.length/2)) : history.length;
  const recent=history.slice(0,recentSize);
  const earlier=history.slice(recent.length,Math.min(recent.length*2,history.length));
  const recentRetention=recent.length ? Math.round(recent.filter(log=>log.rating>=2).length/recent.length*100) : null;
  const earlierRetention=earlier.length ? Math.round(earlier.filter(log=>log.rating>=2).length/earlier.length*100) : null;
  const lapseRate=history.length ? again/history.length : 0;
  const difficultyScore=Math.round(clamp(((progress.difficulty-1)/9)*.62+lapseRate*.38,0,1)*100);
  let label='Em desenvolvimento'; let tone='medium'; let guidance='Continue revisando para o diagnóstico ganhar precisão.';
  if(!history.length) { label='Sem histórico'; tone='neutral'; guidance='A análise começa depois da primeira resposta.'; }
  else if(history.length>=4 && recentRetention>=90 && lapseRate<.2) { label='Bem consolidado'; tone='low'; guidance='O card está saudável; mantenha o agendamento normal.'; }
  else if(earlierRetention!==null && recentRetention>=80 && recentRetention>=earlierRetention+15) { label='Em recuperação'; tone='improving'; guidance='A retenção recente melhorou; mantenha a formulação atual.'; }
  else if(difficultyScore>=65 || lapseRate>=.3) { label='Requer atenção'; tone='high'; guidance='Considere simplificar a pergunta ou dividir o conteúdo em dois cards.'; }
  return {history,total:history.length,remembered,again,retention,recentRetention,earlierRetention,lapseRate,difficultyScore,label,tone,guidance};
}
function flashcardRatingLabel(rating) {
  return ['','Novamente','Difícil','Bom','Fácil'][Math.max(1,Math.min(4,n(rating)))] || 'Resposta';
}
function flashcardHistoryIntervalLabel(log) {
  if(n(log.scheduledDays)>=1) return `${n(log.scheduledDays)} d`;
  if(n(log.scheduledMinutes)>=1) return `${n(log.scheduledMinutes)} min`;
  return 'reaprender';
}
function renderFlashcardHistoryChart(history) {
  const rows=history.slice(0,12).reverse();
  if(!rows.length) return '<div class="fc-history-chart-empty">A evolução dos intervalos aparecerá aqui.</div>';
  const max=Math.max(1,...rows.map(log=>n(log.scheduledDays)));
  return `<div class="fc-history-chart" role="img" aria-label="Evolução dos intervalos das últimas ${rows.length} respostas">${rows.map(log=>{const days=n(log.scheduledDays); const height=days ? Math.max(10,Math.round(days/max*100)) : 5; return `<span title="${escapeAttr(`${flashcardRatingLabel(log.rating)} · ${flashcardHistoryIntervalLabel(log)}`)}"><i class="rating-${log.rating}" style="height:${height}%"></i></span>`;}).join('')}</div>`;
}
function renderFlashcardCardHistory(card) {
  const analysis=flashcardCardHistoryAnalysis(card);
  const visible=analysis.history.slice(0,30);
  const retention=analysis.retention===null?'—':`${analysis.retention}%`;
  return `<section class="fc-card-history"><div class="fc-card-history-head"><div><span class="eyebrow">Memória do card</span><h3>Histórico e dificuldade</h3></div><span class="fc-history-diagnosis tone-${analysis.tone}">${escapeHtml(analysis.label)}</span></div>
    <div class="fc-history-kpis"><div><strong>${analysis.total}</strong><span>respostas</span></div><div><strong>${retention}</strong><span>retenção</span></div><div><strong>${analysis.again}</strong><span>lapsos</span></div><div><strong>${analysis.difficultyScore}</strong><span>dificuldade</span></div></div>
    <div class="fc-history-difficulty"><span><b>Nível de dificuldade</b><strong>${analysis.difficultyScore}/100</strong></span><div><i style="width:${analysis.difficultyScore}%"></i></div><small>${escapeHtml(analysis.guidance)}</small></div>
    <div class="fc-history-chart-shell"><span>Evolução dos intervalos</span>${renderFlashcardHistoryChart(analysis.history)}</div>
    <details class="fc-history-timeline" ${analysis.total&&analysis.total<=5?'open':''}><summary>Linha do tempo <span>${analysis.total}</span></summary>${visible.length?`<ol>${visible.map(log=>{const parsed=Date.parse(log.reviewedAt||''); const when=Number.isFinite(parsed)?new Date(parsed).toLocaleString('pt-BR',{dateStyle:'short',timeStyle:'short'}):'Data não registrada'; return `<li class="rating-${log.rating}"><i></i><div><strong>${escapeHtml(flashcardRatingLabel(log.rating))}</strong><span>${escapeHtml(when)}${log.wasNew?' · primeira revisão':''}</span></div><em>${escapeHtml(flashcardHistoryIntervalLabel(log))}</em></li>`;}).join('')}</ol>${analysis.total>visible.length?`<small>Mais ${analysis.total-visible.length} registros anteriores continuam preservados no histórico.</small>`:''}`:'<div class="fc-history-empty">Este card ainda não foi respondido.</div>'}</details>
  </section>`;
}
function renderFlashcardBrowserDetail(card) {
  ...
  return `<aside class="card fc-browser-detail"><div class="fc-browser-detail-head"><div><span class="eyebrow">${escapeHtml(flashcardBrowserSourceLabel(card))}</span><h2>Detalhes do card</h2></div>${editing?'':`<button class="tiny-btn" data-edit-flashcard="${escapeAttr(card.id)}">Editar</button>`}</div>
    ...
    ${renderFlashcardCardHistory(card)}
  </aside>`;
}
...
function normalizeFlashcardCustomFilters(filters={}) {
  return {
    mode:['overdue','due','difficult','recentErrors','tags','all'].includes(filters.mode)?filters.mode:'overdue',
    area:filters.area || 'Todas',
    subject:filters.subject || 'Todos',
    tag:String(filters.tag || '').trim(),
    recentDays:Math.max(1,Math.min(365,Math.round(n(filters.recentDays)||30))),
    limit:Math.max(1,Math.min(500,Math.round(n(filters.limit)||50)))
  };
}
function normalizeFlashcardSmartDeck(deck={}) {
  const createdAt=deck.createdAt || deck.updatedAt || new Date().toISOString();
  return {
...
function flashcardSmartDecks() {
  return (state.flashcardSettings.smartDecks || []).map(normalizeFlashcardSmartDeck).filter(deck=>deck.name).sort((a,b)=>(Date.parse(b.updatedAt)||0)-(Date.parse(a.updatedAt)||0) || a.name.localeCompare(b.name));
}
function saveFlashcardSmartDeck(name, filters=ui.flashcardCustomFilters, id='') {
  const cleanName=String(name||'').trim().slice(0,80);
  if(!cleanName) return null;
  if(!Array.isArray(state.flashcardSettings.smartDecks)) state.flashcardSettings.smartDecks=[];
  const now=new Date().toISOString();
...
function deleteFlashcardSmartDeck(id) {
...
function flashcardSmartDeckSummary(filters={}) {
  ...
  const modeLabels={overdue:'Atrasados',due:'Vencidos agora',difficult:'Mais difíceis',recentErrors:`Erros em ${clean.recentDays} dias`,tags:'Por tags',all:'Toda a coleção'};
  return [modeLabels[clean.mode],clean.area!=='Todas'?clean.area:'',clean.subject!=='Todos'?clean.subject:'',clean.tag?`#${clean.tag.split(/\s+/).join(' #')}`:'',`até ${clean.limit}`].filter(Boolean).join(' · ');
}
function flashcardRecentAgainByCard(days=30) {
  ...
  (state.flashcardSystem?.reviewLogs || []).forEach(log=>{
    ...
    if(n(log?.rating)!==1 || !Number.isFinite(timestamp) || timestamp<cutoff) return;
    map.set(log.cardId,Math.max(n(map.get(log.cardId)),timestamp));
  });
  return map;
}
function flashcardCustomStudyCandidates(all=flashcardAllRecords(), rawFilters=ui.flashcardCustomFilters) {
  const filters=normalizeFlashcardCustomFilters(rawFilters);
  const today=studyDateKey();
  const recentAgain=flashcardRecentAgainByCard(filters.recentDays);
  const tagTerms=normalizedTopic(filters.tag).split(' ').filter(Boolean);
  const candidates=flashcardActiveRecords(all).filter(card=>{
    const progress=flashcardProgress(card);
    const subject=card.subject || card.subarea || card.topic || 'Sem assunto';
    const areaOk=filters.area==='Todas' || card.area===filters.area;
    const subjectOk=filters.subject==='Todos' || subject===filters.subject;
    const tagText=normalizedTopic((card.tags||[]).join(' '));
    const tagOk=!tagTerms.length || tagTerms.every(term=>tagText.includes(term));
    const modeOk=filters.mode==='all'
      || (filters.mode==='overdue' && progress.reviews>0 && progress.nextReview<today)
      || (filters.mode==='due' && progress.reviews>0 && progress.nextReview<=today)
      || (filters.mode==='difficult' && (progress.difficulty>=7 || progress.lapses>=2 || progress.status==='Difícil'))
      || (filters.mode==='recentErrors' && recentAgain.has(card.id))
      || (filters.mode==='tags' && tagTerms.length>0 && tagOk);
    return areaOk && subjectOk && tagOk && modeOk;
  });
  return candidates.sort((a,b)=>{
    const pa=flashcardProgress(a); const pb=flashcardProgress(b);
    if(filters.mode==='difficult') return pb.difficulty-pa.difficulty || pb.lapses-pa.lapses;
    if(filters.mode==='recentErrors') return n(recentAgain.get(b.id))-n(recentAgain.get(a.id));
...
function flashcardSmartDeckPriority(deck, all=flashcardAllRecords()) {
  const analytics=flashcardSmartDeckAnalytics(deck,all);
  const candidates=flashcardCustomStudyCandidates(all,deck.filters);
  const today=studyDateKey();
  const overdue=candidates.filter(card=>{const progress=flashcardProgress(card); return progress.reviews>0 && progress.nextReview<today;}).length;
  const difficult=candidates.filter(card=>{const progress=flashcardProgress(card); return progress.difficulty>=7 || progress.lapses>=2 || progress.status==='Difícil';}).length;
  const queuePoints=Math.min(20,analytics.candidates);
  const overduePoints=Math.min(35,overdue*2.5);
  const retentionPoints=analytics.retention===null?0:clamp((90-analytics.retention)/90,0,1)*25;
  const errorPoints=analytics.reviews?Math.min(15,analytics.errors/analytics.reviews*30):0;
  const difficultyPoints=Math.min(10,difficult*.75);
  const trendPoints=analytics.trend!==null && analytics.trend<0?Math.min(5,Math.abs(analytics.trend)/4):0;
  const score=Math.round(queuePoints+overduePoints+retentionPoints+errorPoints+difficultyPoints+trendPoints);
  const reasons=[];
  if(overdue) reasons.push(`${overdue} ${overdue===1?'card atrasado':'cards atrasados'}`);
  if(analytics.retention!==null && analytics.retention<90) reasons.push(`retenção em ${analytics.retention}%`);
  if(analytics.errors) reasons.push(`${analytics.errors} ${analytics.errors===1?'erro recente':'erros recentes'}`);
  if(analytics.trend!==null && analytics.trend<0) reasons.push(`queda de ${Math.abs(analytics.trend)} p.p. na semana`);
  if(!reasons.length && difficult) reasons.push(`${difficult} ${difficult===1?'card difícil':'cards difíceis'}`);
  if(!reasons.length) reasons.push(`${analytics.candidates} ${analytics.candidates===1?'card disponível':'cards disponíveis'}`);
  const urgency=score>=55?'high':score>=30?'medium':'normal';
  return {deck:normalizeFlashcardSmartDeck(deck),analytics,score,urgency,overdue,difficult,reasons};
}
function flashcardRecommendedSmartDeck(all=flashcardAllRecords(), options={}) {
  const available=flashcardSmartDecks().map(deck=>flashcardSmartDeckPriority(deck,all)).filter(item=>item.analytics.candidates>0);
  if(!available.length) return null;
  const pinned=available.filter(item=>item.deck.pinned);
...
function renderFlashcardSmartDeckOverview(all=flashcardAllRecords()) {
  const decks=flashcardSmartDecks();
  const recommendation=flashcardRecommendedSmartDeck(all,{preferPinned:true});
  const recommended=recommendation?`<div class="fc-smart-recommended urgency-${recommendation.urgency}"><div><span class="eyebrow">Recomendado agora</span><strong>${escapeHtml(recommendation.deck.name)}</strong><small>${escapeHtml(recommendation.reasons.slice(0,3).join(' · '))}</small></div><div><span><b>${recommendation.analytics.candidates}</b> na fila</span><span><b>${recommendation.overdue}</b> atrasados</span><button class="icon-btn primary" data-fc-smart-start="${escapeAttr(recommendation.deck.id)}">Estudar recomendação</button></div></div>`:decks.length?'<div class="fc-smart-recommended is-clear"><div><span class="eyebrow">Recomendação automática</span><strong>Tudo em dia</strong><small>Nenhum baralho tem cartões disponíveis agora.</small></div></div>':'';
  return `<section class="card fc-smart-performance"><div class="section-title"><div><span class="eyebrow">Desempenho por regra</span><h2>Baralhos inteligentes</h2><div class="muted">Retenção, erros e ritmo dos últimos 30 dias. Fixe os mais importantes no Painel.</div></div><button class="icon-btn" data-fc-view="custom">Gerenciar</button></div>${recommended}${decks.length?`<div class="fc-smart-deck-grid">${decks.map(deck=>renderFlashcardSmartDeckCard(deck,all)).join('')}</div>`:'<div class="fc-smart-empty">Crie seu primeiro baralho inteligente em Personalizar.</div>'}</section>`;
}
function renderFlashcardCustomStudy(all) {
  const filters=normalizeFlashcardCustomFilters(ui.flashcardCustomFilters);
  ui.flashcardCustomFilters=filters;
  const areas=['Todas',...new Set(all.map(card=>card.area).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
  if(!areas.includes(filters.area)) filters.area='Todas';
  const areaCards=filters.area==='Todas'?all:all.filter(card=>card.area===filters.area);
  const subjects=['Todos',...new Set(areaCards.map(card=>card.subject||card.subarea||card.topic).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
  if(!subjects.includes(filters.subject)) filters.subject='Todos';
  const candidates=flashcardCustomStudyCandidates(all,filters);
  const smartDecks=flashcardSmartDecks();
  const editingDeck=smartDecks.find(deck=>deck.id===ui.flashcardCustomEditingDeckId) || null;
  if(ui.flashcardCustomEditingDeckId && !editingDeck) ui.flashcardCustomEditingDeckId='';
  const modes=[['overdue','Atrasados','Cards cuja revisão deveria ter acontecido antes de hoje.'],['due','Vencidos agora','Inclui atrasados e os programados para hoje.'],['difficult','Mais difíceis','Prioriza dificuldade alta, lapsos e respostas difíceis.'],['recentErrors','Erros recentes',`Cards respondidos como “Novamente” nos últimos ${filters.recentDays} dias.`],['tags','Por tags','Monta a fila usando as tags informadas abaixo.'],['all','Toda a coleção','Usa área, assunto, tag e limite para recortar a coleção.']];
...
function renderFlashcards() {
  ...
  document.querySelectorAll('[data-fc-custom-mode]').forEach(button => button.onclick = event => {
    ui.flashcardCustomFilters={...normalizeFlashcardCustomFilters(ui.flashcardCustomFilters),mode:event.currentTarget.dataset.fcCustomMode};
    renderFlashcards();
  });
  document.querySelectorAll('[data-fc-custom-filter]').forEach(input => input.onchange = event => {
    const key=event.currentTarget.dataset.fcCustomFilter;
    const value=['recentDays','limit'].includes(key)?n(event.currentTarget.value):event.currentTarget.value;
    ui.flashcardCustomFilters=normalizeFlashcardCustomFilters({...ui.flashcardCustomFilters,[key]:value,...(key==='area'?{subject:'Todos'}:{})});
    renderFlashcards();
  });
  ...
  document.getElementById('fcSmartDeckSave')?.addEventListener('click',()=>{
    ...
    const deck=saveFlashcardSmartDeck(name,ui.flashcardCustomFilters,ui.flashcardCustomEditingDeckId);
    if(!deck) { alert('Dê um nome ao baralho inteligente.'); return; }
    ui.flashcardCustomEditingDeckId='';
    persist();
    showStudyToast?.(`Baralho “${deck.name}” salvo.`);
    renderFlashcards();
  });
  document.getElementById('fcSmartDeckCancel')?.addEventListener('click',()=>{ ui.flashcardCustomEditingDeckId=''; renderFlashcards(); });
  document.querySelectorAll('[data-fc-smart-edit]').forEach(button=>button.onclick=event=>{
    const deck=flashcardSmartDecks().find(item=>item.id===event.currentTarget.dataset.fcSmartEdit);
    if(!deck) return;
    ui.flashcardCustomFilters={...deck.filters};
    ui.flashcardCustomEditingDeckId=deck.id;
    renderFlashcards();
  });
  document.querySelectorAll('[data-fc-smart-start]').forEach(button=>button.onclick=event=>{
  ...
  document.querySelectorAll('[data-fc-smart-delete]').forEach(button=>button.onclick=event=>{
    ...
    const deck=flashcardSmartDecks().find(item=>item.id===id);
    if(!deck || !confirm(`Excluir o baralho inteligente “${deck.name}”? Os cards e o histórico não serão apagados.`)) return;
    deleteFlashcardSmartDeck(id);
    if(ui.flashcardCustomEditingDeckId===id) ui.flashcardCustomEditingDeckId='';
    persist();
    renderFlashcards();
  });
  ...
  document.querySelectorAll('[data-fc-heat-date]').forEach(button => button.onclick = event => {
    ...
    const row=flashcardReviewCalendar(flashcardReviewDailySeries(),all).find(item=>item.date===date) || {kind:'past',completed:0,due:0,again:0,remembered:0,minutes:0};
    const rated=row.again+row.remembered;
    const detail=row.kind==='future'
      ? `${row.due} ${row.due===1?'card previsto':'cards previstos'} para revisar`
      : [`${row.completed} ${row.completed===1?'card revisado':'cards revisados'}`,rated?`${Math.round(row.remembered/rated*100)}% lembrados`:'',row.minutes?`${Math.round(row.minutes)} min`:'',row.kind==='today'?`${row.due} ainda pendentes`:'' ].filter(Boolean).join(' · ');
    showStudyToast?.(`${fmtDate(date)} · ${detail}`);
  });
  const heatShell=document.querySelector('.fc-heatmap-shell');
  ...
  document.querySelectorAll('[data-flashcard-lesson-deck]').forEach(button => button.onclick = e => {
    ui.flashcardLesson = e.currentTarget.dataset.flashcardLessonDeck;
    ui.flashcardSubject=''; ui.flashcardDeck=''; ui.flashcardIndex=0; ui.flashcardSessionDone=false; ui.revealedCards={};
    renderFlashcards();
  });
  document.getElementById('flashcardClearLesson')?.addEventListener('click', () => {
    ui.flashcardLesson=''; ui.flashcardIndex=0; ui.flashcardSessionDone=false; ui.revealedCards={}; renderFlashcards();
  });
  document.getElementById('flashcardReleaseAll')?.addEventListener('click', () => {
    const ids = flashcardAllRecords().filter(flashcardIsWaiting).map(card => card.id);
    if(!ids.length) return;
    if(!confirm(`Liberar ${ids.length} ${ids.length===1?'card':'cards'} da espera de uma vez? Eles passam a entrar na fila conforme o limite de revisões.`)) return;
    releaseFlashcards(ids);
    persist();
    renderFlashcards();
  });
...
// O banco completo tem dezenas de milhares de questões e centenas de MB. Ele é
// carregado sob demanda por ensureViewData somente nas telas que o utilizam;
// pré-carregá-lo aqui bloqueava a navegação e fazia a Missão levar ~30 s.
