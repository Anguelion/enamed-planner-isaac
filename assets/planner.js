...
function dayRoadItems(date) {
  ...
  return [
    { id: 'daily-video', scheduleId: lessons[0]?.id || '', type: 'Videoaulas', icon: '▶', label: lessonLabel, done: videosDone >= videoTarget, progress: videosDone, target: videoTarget, unit: videoTarget === 1 ? 'aula' : 'aulas', foot: lessons.length ? `Bloco ${lessons[0].block} · ${dayVideoFiles.length ? `${dayVideoFiles.length} arquivos disponíveis` : lessons.map(item=>item.area).filter(Boolean).slice(0,2).join(' / ')}` : 'Aulas programadas para hoje' },
    { id: 'daily-questions', scheduleId: lessons[0]?.id || '', type: 'Questões', icon: 'Q', label: `${questionTarget} questões`, done: n(log.questions) >= questionTarget, progress: n(log.questions), target: questionTarget, unit: 'questões', foot: n(log.correct)+n(log.wrong) ? `${pct(dayScore(log))} de aproveitamento` : 'Meta diária' },
    { id: 'daily-flashcards', scheduleId: lessons[0]?.id || '', type: 'Flashcards', icon: 'FC', label: `${flashcardTarget} flashcards`, done: n(log.flashcards) >= flashcardTarget, progress: n(log.flashcards), target: flashcardTarget, unit: 'flashcards', foot: 'Meta diária de revisão' }
  ];
}
