...
function renderAulas() {
  ...
  if(renderedVideo) {
    ...
    cleanFrameToggle.innerHTML='<span>↩ Restaurar controles</span>';
  ...
  const setVideoFocusMode = enabled => {
    ...
    const restoreButton=document.getElementById('restoreVideoLayout');
    layout?.classList.toggle('video-focus-mode',ui.videoFocusMode);
    [button, restoreButton].forEach(control => {
      if(!control) return;
      control.setAttribute('aria-hidden', String(!ui.videoFocusMode && control === restoreButton));
    });
    if(button) {
      ...
      button.innerHTML=ui.videoFocusMode?'Voltar ao layout completo':`${iconSvg('focus',{weight:'regular'})} Foco`;
    }
  };
  document.getElementById('toggleVideoFocus')?.addEventListener('click', () => setVideoFocusMode(!ui.videoFocusMode));
  document.getElementById('restoreVideoLayout')?.addEventListener('click', () => setVideoFocusMode(false));
