...
// Nesta fase, o planner é local-first: não depende de Supabase nem de rede.
const OFFLINE_FIRST = true;
const sbClient = null;
...
function renderBlockStrip() {
  ...
  return `<div class="block-strip"><button class="block-chip ${ui.scheduleBlock==='Todos'?'active':''}" data-schedule-block="Todos">Todos</button><button class="block-chip ${ui.scheduleBlock==='Atual'?'active':''}" data-schedule-block="Atual">Atual</button>${scheduleBlocks().map(block => {
    const status = blockStatus(block);
    return `<button class="block-chip ${String(ui.scheduleBlock)===block?'active':''} ${status}" title="${escapeAttr(blockStatusTitle(status))}" data-schedule-block="${escapeAttr(block)}">${escapeHtml(block)}</button>`;
  }).join('')}</div>`;
}
...
function renderCronograma() {
  ...
  document.getElementById('cronograma').innerHTML = `<div class="card"><div class="section-title"><div><h2>Blocos do cronograma</h2><div class="muted">Bloco atual estimado: ${escapeHtml(String(currentScheduleBlock() || '-'))}${changed ? ` · última alteração: ${escapeHtml(changed.topic)} (${fmtDate(changed.date)})` : ''}</div></div></div>${renderBlockStrip()}<div class="toolbar"><input class="input" id="search" placeholder="Buscar tema, área, data..." value="${escapeAttr(ui.search)}"><select class="select" id="areaFilter">${areas.map(a=>`<option ${a===ui.area?'selected':''}>${escapeHtml(a)}</option>`).join('')}</select><select class="select" id="statusFilter">${['Todos','Concluído','Aguardando','Não Iniciado'].map(s=>`<option ${s===ui.status?'selected':''}>${s}</option>`).join('')}</select><button class="icon-btn" id="clearFilters">Limpar filtros</button></div></div><div class="card"><div class="section-title"><h2>Cronograma editável</h2><span class="muted">${rows.length} itens</span></div>${renderScheduleTable(rows, true)}</div>`;
...
function renderMateriais() {
  const groups = materialLibraryGroups();
  ...
  document.getElementById('materiais').innerHTML = `<div class="library-overview"><div><span class="eyebrow">Biblioteca médica</span><h2>Resumos para revisar sem sair do planner</h2><p>Texto pesquisável, organizado por aula e acompanhado apenas das figuras essenciais.</p></div><div class="library-stats"><span><strong>${materialLibrary.length}</strong> resumos</span><span><strong>${new Set(materialLibrary.filter(doc=>doc.scheduleId).map(doc=>doc.scheduleId)).size}</strong> aulas</span><span><strong>${figures}</strong> figuras</span></div></div><div class="library-layout"><aside class="card library-sidebar"><div class="section-title"><div><span class="eyebrow">Navegação</span><h2>Materiais por aula</h2></div><span class="badge today">${groups.length}</span></div><label class="search-field"><span aria-hidden="true">⌕</span><input class="input" id="materialSearch" value="${escapeAttr(ui.materialSearch)}" placeholder="Buscar tema, título ou subtítulo"></label><div class="library-folders">${groups.map(group => renderMaterialFolder(group, selected?.id)).join('')}</div></aside><section class="card material-reader">${renderMaterialReader(selected)}</section></div>`;
  ...
  document.querySelectorAll('[data-material-doc]').forEach(button => button.onclick = e => { ui.materialDocId=e.currentTarget.dataset.materialDoc; renderMateriais(); });
...
function renderQuestion(question, total) {
  ...
  const feedback = result ? `<div class="question-feedback ${result.correct?'':'wrong'}"><strong>${result.correct?'Resposta correta.':'Resposta incorreta.'}</strong>${timeoutText} Gabarito: ${question.answer}.${!result.correct ? ' Marque este assunto para revisão.' : ''}</div>` : '';
